...
vector<int> analysis(vector<string> def_class, vector<int> brack_ind, int num_of_row) {
	...
	for (int i = 0; i < size3; i = i + 2) {
		int pos1, pos2, pos8, pos9, pos10;
		ind_of_comm = def_class[num_of_row].find(',', brack_ind[i]);
		if (def_class[num_of_row][ind_of_comm - 1] == 'L' || def_class[num_of_row][ind_of_comm - 1] == 'l') {
			pos1 = def_class[num_of_row].find('e', brack_ind[i]);
			pos2 = def_class[num_of_row].find('.', brack_ind[i]);
			if (pos1 == -1 && pos2 == -1) {
				var1 = true;
			}
			else if (pos1 > ind_of_comm && pos2 > ind_of_comm){
				var1 = true;
			}
			else if (pos1 > ind_of_comm && pos2 == -1) {
				var1 = true;
			}
			else if (pos1 == -1 && pos2 > ind_of_comm) {
				var1 = true;
			}
			else {
				var1 = false;
		...
		if (def_class[num_of_row][brack_ind[i + 1] - 1] == 'l' || def_class[num_of_row][brack_ind[i + 1] - 1] == 'L' || def_class[num_of_row][ind_of_comm + 2] == 'u' || def_class[num_of_row][ind_of_comm + 2] == 'U') {
			pos8 = def_class[num_of_row].find('-', ind_of_comm);
			pos9 = def_class[num_of_row].find('e', ind_of_comm);
			pos10 = def_class[num_of_row].find('.', ind_of_comm);

			if (pos8 == -1 && pos9 == -1 && pos10 == -1) {
				var2 = true;
			}
			else if ((pos8 > ind_of_comm && pos8 < brack_ind[i + 1]) || (pos9 > ind_of_comm && pos9 < brack_ind[i + 1]) || (pos10 > ind_of_comm && pos10 < brack_ind[i + 1])) {
				var2 = false;
			}
			else {
				var2 = true;
...
void function(vector<string> sym, vector<int> arr1, vector<int> arr2, int len) {
	...
	for (int i = 0; i < size5; ++i) {
		...
		//output(num_brack, num_brack.size());
		if (!ind) {
			if (numbers.size() / 2 == 1) {
